...
# Read the Excel file
file_path = '/home/ra/Downloads/S-02 OUTSTANDING.xlsx'
...
def populate_models_from_excel(data):
    ...
    # outstanding_in = CustomerOutstanding.objects.filter(customer__routes__route_name="S-02",product_type='amount')
    # Invoice.objects.filter(customer__routes__route_name="S-02").delete()
    # outstanding_in.delete()
    # CustomerOutstandingReport.objects.filter(customer__routes__route_name="S-02",product_type='amount').delete()
    ...
    for index, row in data.iterrows():
        ...
        routes = RouteMaster.objects.exclude(route_name__in=["test2","test_route"])
        for route in routes:
            branch = BranchMaster.objects.get(user_id__username="ajman")
            Customers.objects.filter(routes=route).update(branch_id=branch)
