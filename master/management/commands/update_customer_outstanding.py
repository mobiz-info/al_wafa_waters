...
# Read the Excel file
file_path = '/home/ra/Downloads/S-01uploadlist.xlsx'
...
def populate_models_from_excel(data):
    user = CustomUser.objects.get(username="S-01")
    # date = datetime.strptime("2024-11-12", '%Y-%m-%d')
    
    # outstanding_in = CustomerOutstanding.objects.filter(customer__routes__route_name="S-01",product_type='amount')
    # Invoice.objects.filter(customer__routes__route_name="S-01").delete()
    ...
    # CollectionPayment.objects.filter(customer__routes__route_name="S-01").delete()
    ...
    for index, row in data.iterrows():
        ...
        routes = RouteMaster.objects.get(route_name="S-01")
